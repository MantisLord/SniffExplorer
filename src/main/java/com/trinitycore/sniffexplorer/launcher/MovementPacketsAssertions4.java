...
public class MovementPacketsAssertions4 {

    protected static final Logger logger = LoggerFactory.getLogger(MovementPacketsAssertions4.class);
...
//    private static final String INPUT_SNIFF ="C:\\Sniffs\\335\\bin\\BINSlavePens_parsed.txt";
    ...
    private static void processFile(File file) {
        Set<OpCode> ignoredOpCodes = new HashSet<>();
        ignoredOpCodes.add(SMSG_COMPRESSED_UPDATE_OBJECT);
        ignoredOpCodes.add(SMSG_ON_MONSTER_MOVE);
        CriteriaSet criteriaSet = new CriteriaSet(message -> {
            return !ignoredOpCodes.contains(message.getOpCode()) && message.getMessageText().stream().anyMatch(line -> line.contains("WalkMode"));
        });
