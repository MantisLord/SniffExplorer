 ...
 */
public class MovementPacketsAssertions {

    protected static final Logger logger = LoggerFactory.getLogger(MovementPacketsAssertions.class);
...
//    private static final String INPUT_SNIFF ="C:\\Sniffs\\335\\bin\\BINSlavePens_parsed.txt";
    ...
    private static void processFile(File file) {
        ...
        opCodesToAnalyse.add(OpCode.CMSG_MOVE_SET_COLLISION_HGT_ACK);
        opCodesToAnalyse.add(OpCode.SMSG_MOVE_SET_COLLISION_HGT);
...
//            boolean conditionToAssert = getUnit(message) instanceof Player && message.getDirection().equals(Direction.ServerToClient);
            boolean conditionToAssert = getUnit(message) instanceof Player;
            if(!conditionToAssert){
                viewer.show(message);
                System.out.println("condition not verified in " + file);
//                viewer.close();
